...
@export_range(0.0, 24.0, 0.1) var fixed_time := 0.0


var times := {
	0.0: TimeOfDay.new(0.8, 0.0, Vector3.ZERO, 7.0, 100.0, Color(0.158, 0.183, 0.27, 1.0), 1.15),
	3.0: TimeOfDay.new(0.8, 0.0, Vector3(0.0, 360.0, 0.0), 7.0, 100.0, Color(0.354, 0.398, 0.546, 1.0), 1.1),
	7.0: TimeOfDay.new(0.7, 0.7, Vector3(-27, 320.0, 0.0), 8.0, 120.0, Color(0.662, 0.701, 0.822, 1.0), 1.05),
	...
	22.0: TimeOfDay.new(0.8, 0.0, Vector3(-25, 200.0, 0.0), 8.0, 120.0, Color(0.245, 0.279, 0.398, 1.0), 1.07),
}
...
func _update() -> void:
	...
	var first_key
	var second_key
	var hour := GameState.game_data.time if real_time else fixed_time
	...
	var interpolation_ratio = (hour - first_key) / (second_key - first_key)
